...
int playback(const char* fname, int t_offset, IplImage* gimgf)
{
	...
	if (getline(fin, str))
	{
		str.erase(str.size()-1); // to eraise '/r' character at the end
		name = dir+str;

		
		IplImage* img = cvLoadImage(name.c_str(),CV_LOAD_IMAGE_COLOR);
		...
		if (p == string::npos || q == string::npos) {
			...
			printf("Error: Reading timestamp from image file name\n");
			return -1;
		}
		string str = name.substr(p+1, q-p-1);
		time_msec = atoi(str.c_str()) + t_offset;

		printf("This means something: %s, %d\n", str.c_str(), time_msec);
